...
export class MemStorage implements IStorage {
  ...
  async getChatHistory(userId: number): Promise<ChatHistory | undefined> {
    // Find the chat history directly by iterating through entries and checking keys that start with userId:
  ...
  async createChatHistory(chatHistory: InsertChatHistory): Promise<ChatHistory> {
    ...
    // Use composite key with userId to ensure data isolation
    this.chatHistoryMap.set(`${chatHistory.userId}:${id}`, history);
    return history;
  }
